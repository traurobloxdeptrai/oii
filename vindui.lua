...
NullUI.Theme = {
	Background     = Color3.fromRGB(29, 37, 54),
	Surface        = Color3.fromRGB(43, 55, 78),
	Text           = Color3.fromRGB(248, 250, 255),
	TextDim        = Color3.fromRGB(196, 204, 220),
	Accent         = Color3.fromRGB(156, 205, 255),
	Danger         = Color3.fromRGB(255, 148, 166),
...
local function GlassLayer(parent, radius, transparency)
	...
	glass.BackgroundColor3 = Color3.fromRGB(218, 233, 255)
	glass.BackgroundTransparency = transparency or 0.94
	...
	sheen.Name = "SpecularHighlight"
	sheen.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	sheen.BackgroundTransparency = 0.82
	sheen.BorderSizePixel = 0
	sheen.Size = UDim2.new(1, 0, 0.46, 0)
	sheen.ZIndex = Z.Glass
	sheen.Parent = glass
	Corner(sheen, radius)
	local sheenGradient = Instance.new("UIGradient")
	sheenGradient.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.08), NumberSequenceKeypoint.new(1, 1) })
	sheenGradient.Rotation = 90
	sheenGradient.Parent = sheen

	local prism = Instance.new("Frame")
	prism.Name = "RefractionPrism"
	prism.AnchorPoint = Vector2.new(0.5, 0.5)
	prism.BackgroundColor3 = Color3.fromRGB(190, 220, 255)
	prism.BackgroundTransparency = 0.9
	prism.BorderSizePixel = 0
	prism.Position = UDim2.fromScale(0.5, 0.5)
	...
	prismGradient.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 178, 236)),
		ColorSequenceKeypoint.new(0.48, Color3.fromRGB(190, 240, 255)),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(166, 182, 255)),
	})
	prismGradient.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.35), NumberSequenceKeypoint.new(0.5, 0.72), NumberSequenceKeypoint.new(1, 1) })
	prismGradient.Rotation = 35
	prismGradient.Parent = prism

	local prismEdge = Instance.new("Frame")
	prismEdge.Name = "RefractionEdge"
	prismEdge.AnchorPoint = Vector2.new(0.5, 0.5)
	prismEdge.BackgroundColor3 = Color3.fromRGB(234, 248, 255)
	prismEdge.BackgroundTransparency = 0.94
	prismEdge.BorderSizePixel = 0
	prismEdge.Position = UDim2.fromScale(0.5, 0.5)
	...
	local function updateRefraction(pointer)
		...
		local x = math.clamp((pointer.X - position.X) / size.X, 0, 1)
		local y = math.clamp((pointer.Y - position.Y) / size.Y, 0, 1)
		prism.Position = UDim2.fromScale(x, y)
		prismEdge.Position = UDim2.fromScale(math.clamp(x + 0.035, 0, 1), math.clamp(y - 0.03, 0, 1))
		prismGradient.Rotation = 20 + x * 70 - y * 24
	end
	updateRefraction(UserInputService:GetMouseLocation())
	local refractionConnection = UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
			updateRefraction(input.Position)
		end
	end)
	glass.Destroying:Connect(function() refractionConnection:Disconnect() end)
	return glass
end
...
local function BaseCard(parent, height)
	...
	card.BackgroundTransparency = 0.76
	card.BorderSizePixel = 0
	card.Size = UDim2.new(1, 0, 0, height or 44)
	card.ZIndex = Z.Content
	card.Parent = parent
	Corner(card, 14)
	Stroke(card, Color3.fromRGB(235, 243, 255), 1, 0.76)
	local cardGradient = Instance.new("UIGradient")
	cardGradient.Color = ColorSequence.new(Color3.fromRGB(255, 255, 255), Color3.fromRGB(186, 213, 255))
	cardGradient.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.42), NumberSequenceKeypoint.new(1, 0.84) })
...
function NullUI:CreateWindow(opts)
	...
	main.Size = size
	main.BackgroundColor3 = Color3.fromRGB(24, 31, 46)
	...
	main.Parent = root
	Corner(main, 32)
	Stroke(main, Color3.fromRGB(238, 246, 255), 1, 0.64)
	GlassLayer(main, 32, 0.9)
	...
	toolbar.Name = "LiquidToolbar"
	toolbar.Active = false
	toolbar.BackgroundColor3 = Color3.fromRGB(221, 235, 255)
	toolbar.BackgroundTransparency = 0.68
	toolbar.BorderSizePixel = 0
	toolbar.Position = UDim2.fromOffset(margin, 7)
	toolbar.Size = UDim2.new(1, -margin * 2, 0, 50)
	toolbar.ZIndex = Z.Window
	toolbar.Parent = topbar
	Corner(toolbar, 22)
	Stroke(toolbar, Color3.fromRGB(255, 255, 255), 1, 0.56)
	local toolbarGradient = Instance.new("UIGradient")
	toolbarGradient.Color = ColorSequence.new(Color3.fromRGB(248, 252, 255), Color3.fromRGB(171, 204, 255))
	toolbarGradient.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.34), NumberSequenceKeypoint.new(0.55, 0.78), NumberSequenceKeypoint.new(1, 0.46) })
	toolbarGradient.Rotation = 18
	toolbarGradient.Parent = toolbar
 
	local controlsHolder = Instance.new("Frame")
	controlsHolder.Name = "WindowControls"
	controlsHolder.AnchorPoint = Vector2.new(1, 0.5)
	controlsHolder.Position = UDim2.new(1, -10, 0.5, 0)
	controlsHolder.Size = UDim2.fromOffset(120, 26)
	controlsHolder.BackgroundTransparency = 1
	controlsHolder.ZIndex = Z.Content + 1
	controlsHolder.Parent = toolbar
	...
	titleLabel.TextYAlignment = Enum.TextYAlignment.Center
	titleLabel.TextTruncate = Enum.TextTruncate.AtEnd
	titleLabel.Position = UDim2.fromOffset(titleStartX, opts.Subtitle and 5 or 15)
	...
	if opts.Subtitle then
		...
		subLabel.Position = UDim2.fromOffset(titleStartX, 27)
		subLabel.Size = UDim2.new(1, -titleStartX - titleTextPad, 0, 14)
		subLabel.ZIndex = Z.Content + 1
		subLabel.Parent = toolbar
	end
 
	local tabBar = Instance.new("ScrollingFrame")
	tabBar.Name = "TabBar"
	tabBar.BackgroundColor3 = Color3.fromRGB(225, 238, 255)
	tabBar.BackgroundTransparency = 0.8
	tabBar.BorderSizePixel = 0
	tabBar.Position = UDim2.fromOffset(margin, 70)
	tabBar.Size = UDim2.new(0, 130, 1, -(70 + margin))
	...
	tabBar.Parent = main
	Corner(tabBar, 20)
	Stroke(tabBar, Color3.fromRGB(255, 255, 255), 1, 0.7)
	local tabGlassGradient = Instance.new("UIGradient")
	tabGlassGradient.Color = ColorSequence.new(Color3.fromRGB(244, 249, 255), Color3.fromRGB(175, 205, 255))
	tabGlassGradient.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.45), NumberSequenceKeypoint.new(1, 0.82) })
	tabGlassGradient.Rotation = 90
	tabGlassGradient.Parent = tabBar
	...
	divider.Position = UDim2.new(0, margin + 144, 0, 70)
	divider.Size = UDim2.new(0, 1, 1, -(70 + margin))
	divider.ZIndex = Z.Content
	divider.Parent = main
 
	local contentX = margin + 144 + 16
	local contentSurface = Instance.new("Frame")
	contentSurface.Name = "LiquidContentSurface"
	...
	contentGradient.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.55), NumberSequenceKeypoint.new(1, 0.9) })
	contentGradient.Rotation = 90
	contentGradient.Parent = contentSurface
	local content = Instance.new("Frame")
	content.Name = "Content"
	content.BackgroundTransparency = 1
	content.Position = UDim2.new(0, contentX, 0, 70)
	content.Size = UDim2.new(1, -contentX - margin, 1, -(70 + margin))
	...
	Tween(main, { BackgroundTransparency = 0.34 }, 0.6, Enum.EasingStyle.Exponential)
	Tween(windowScale, { Scale = 1 }, 0.48, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
...
function Window:Close()
	...
	if self._scale then Tween(self._scale, { Scale = 0.88 }, 0.26, Enum.EasingStyle.Quint, Enum.EasingDirection.In) end
...
function Window:Open()
	...
	local gui = self._gui
	gui.Visible = true
	if self._scale then self._scale.Scale = 0.88 end
	...
	if self._scale then Tween(self._scale, { Scale = 1 }, 0.36, Enum.EasingStyle.Back, Enum.EasingDirection.Out) end
...
function Window:AddDockButton(opts)
	...
	if not self._dock then
		local shrunkSize = UDim2.new(0, 130, 1, -(70 + margin + DOCK_HEIGHT + 10))
...
function Tab:AddSection(textOrOpts, maybeIcon)
	...
	if iconAsset ~= "" then
		...
		img.Position = UDim2.fromOffset(2, 5)
		img.ZIndex = Z.Content + 1
		img.Parent = holder
		x = 2 + 13 + 7
	end
	...
	label.Position = UDim2.fromOffset(x, 5)
